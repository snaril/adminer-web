    ...
    <vxe-grid id="rechargerGridTable" ref="userGridRef" @checkbox-change="handleCheckboxChange"
      ...
      <template v-slot:status="{ row }">
        <div style="width: 100%; height: 100%" class="flex justify-center items-center">
          <el-tag v-if="row.status === 2" type="success" style="width: 56px;">已支付</el-tag>
          <el-tag v-else type="info" style="width: 56px;">未支付</el-tag>
          <span></span>
        </div>
      ...
      <template v-slot:Amount="{ row }">
        <div style="width: 100%; height: 100%" class="flex justify-center items-center">
          <el-tag v-if="row.amount < 588 && row.amount" type="danger" style="width: 56px;">{{ row.amount }}</el-tag>
          <el-tag v-else type="success" style="width: 56px;">{{ row.amount }}</el-tag>
      ...
      <template v-slot:arrivedAmount="{ row }">
        <div style="width: 100%; height: 100%" class="flex justify-center items-center">
          <el-tag v-if="row.amount !== row.arrived_amount && row.arrived_amount" type="info" style="width: 56px;">{{ row.arrived_amount }}</el-tag>
          <el-tag v-else-if="row.amount == row.arrived_amount" type="success" style="width: 56px;">{{ row.arrived_amount }}</el-tag>
...
export default {
    ...
    return {
      ...
      gridOptions: {
        ...
        columns: [
          ...
          { field: 'pay_id', title: '支付通道', width: 100, align: 'center' },
          { field: 'amount', title: '充值金额', align: 'center',slots: { default: 'Amount' } },
          { field: 'arrived_amount', title: '实际到账', width: 100, align: 'center',slots: { default: 'arrivedAmount' } },
        ...
        proxyConfig: {
          ...
          ajax: {
            query: ({ page, sorts, filters, form }) => {
              ...
              return rechargeOrderList(queryParams)
  ...
  methods: {
    ...
    fetchUserData({ page, size, sort, filters }) {
      ...
      return rechargeOrderList(page, sort, filters);
    },
    editEvent(row) {
      this.selectRow = true

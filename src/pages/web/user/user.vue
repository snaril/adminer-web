    ...
    <vxe-grid id="rechargerGridTable" ref="userGridRef" @checkbox-change="handleCheckboxChange"
      ...
      <template v-slot:status="{ row }">
        <div style="width: 100%; height: 100%" class="flex justify-center items-center">
          <el-tag v-if="row.status === 1 " type="info" style="width: 56px;">未领取</el-tag>
          <el-tag v-if="row.status === 2" type="success" style="width: 56px;">已领取</el-tag>
          <span></span>
        </div>

        <!-- <el-image  :src="row.image_url" alt="商品图片"  fit="scale-down"/> 设置你希望的图片大小 -->
      </template>
      <template v-slot:recharge="{ row }">
        <div style="width: 100%; height: 100%" class="flex justify-center items-center">
          <el-tag v-if="row.recharge_status === 1 " type="success" style="width: 56px;">是</el-tag>
          <el-tag v-else-if="row.recharge_status === 2" type="success" style="width: 56px;">复购</el-tag>
          <el-tag v-else type="info" style="width: 56px;">否</el-tag>
          <span></span>
        </div>
      ...
      <template v-slot:parent_phone="{ row }">
        <div style="width: 100%; height: 100%" class="flex justify-center items-center bg-green-400">
          <el-tag  type="info" style="width: 100%;">{{row.parent_phone}}</el-tag>
          <span></span>
        </div>

      </template>
      <template #toolbar_buttons>
        <vxe-input v-model="searchName" class="my-search" placeholder="输入手机号码">
          <template #prefix>
            <div class="icon-box">
...
export default {
    ...
    return {
      ...
      gridOptions: {
        ...
        columns: [
          ...
          { field: 'parent_phone', title: '推荐人', width: 120, align: 'center' ,slots: { default: 'parent_phone' } },
          { field: 'code', title: '邀请码', align: 'center' },
          { field: 'quantity', title: '账户余额', align: 'center' },

          { field: 'recharge_status', title: '是否充值', align: 'center', align: 'center',slots: { default: 'recharge' }  },

          { field: 'status', title: '状态', align: 'center', slots: { default: 'status' }, },
        ...
        proxyConfig: {
          ...
          ajax: {
            query: ({ page, sorts, filters, form }) => {
              ...
              return WebUserList(queryParams)
  ...
  methods: {
    ...
    fetchUserData({ page, size, sort, filters }) {
      ...
      return WebUserList(page, sort, filters);
    },
    editEvent(row) {
      this.selectRow = true

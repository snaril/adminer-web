<template>
  <vxe-modal :modelValue="showModal" @update:modelValue="$emit('update:showModal', $event)" :title="'权限配置'" width="300"
    min-width="300" min-height="300" showFooter :loading="submitLoading" resize destroy-on-close>
    <template #default>
...
export default {
  ...
  emits: ['data-to-send',"update:showModal"],
  methods: {
    getCheckedValues() {
      this.showModal = true
    },
    submitEvent() {
      ...
      checkedNodes.forEach(node =>{
        sendArr.push(node.id)
      }) 
      this.$emit('data-to-send', sendArr);
    ...
    setInitialCheckedNodes() {
      const setNodeChecked = (nodes, checkedKeys) => {
        nodes.forEach(node => {
          // 检查节点是否应该被勾选
          const shouldBeChecked = checkedKeys.includes(node.id);
          // 如果节点在 checkedKeys 中，则勾选它
          if (shouldBeChecked) {
            this.$refs.treeRef.setChecked(node, true, false);
          }
          // 如果节点有子节点，递归勾选
          if (node.children && node.children.length) {
            setNodeChecked(node.children, checkedKeys);
          }
        });
      };
      // 确保 defaultCheckedKeys 是一个数组
      const checkedKeys = Array.isArray(this.formData.defaultCheckedKeys) ? this.formData.defaultCheckedKeys : [];
      // 从根节点开始递归
      setNodeChecked(this.formData.node, checkedKeys);
    },
  },
  mounted() {
    this.$nextTick(() => {
